...
void Command::handleNick(Server& server, Client& client, const IRCMessage& msg)
{
	...
	string nicklenStr = server.getIsupport("NICKLEN");
	std::stringstream ss(nicklenStr);
	size_t maxNickLen = 0;
	ss >> maxNickLen;
	string targetNick = msg.params[0];
    if (maxNickLen > 0 && targetNick.length() > maxNickLen)
    	targetNick.erase(maxNickLen);
	if (!checkNickname(targetNick))
	{
		client.sendMessage(ERR_ERRONEUSNICKNAME(client.getName(TYPE_NICK), msg.params[0]));
		return;
	}
	if (nicknameInUse(server, targetNick))
	{
	...
	if (!client.isFullyRegistered())
	{
		client.setName(TYPE_NICK, targetNick);
	...
	else
	{
		...
		client.setName(TYPE_NICK, targetNick);
		/*BURAYA ASLINDA SENDMESSAGE DEĞİL
			server.broadcastToChannelsOf() gelecek*/
