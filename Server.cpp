...
// olabilir; gercek removeClient() orada guvenli degildir. std::set fd'yi
...
void Server::acceptClients()
{
    while (true)
    {
        ...
        try {
            ...
            std::cerr << "client fd=" << fd << " setup failed: " << e.what() << std::endl;
            close(fd);
            continue;
        }
        addPollFd(fd, POLLIN);
...
// fd'ler std::set ile tekillestirilir; degisen kullanici event'i en sonda ve
...
{
    std::string nickMsg = oldPrefix + " NICK " + client.getName(TYPE_NICK) + "\r\n";
    std::set<int> recipients;

    for (std::map<std::string, Channel>::const_iterator cit = _channels.begin();
        cit != _channels.end(); ++cit)
    {
        if (!cit->second.isMember(&client))
            continue;
        const std::map<int, Client*>& members = cit->second.getMembers();
    ...
    {
        std::map<int, Client>::iterator rit = _clients.find(*it);
        if (rit != _clients.end())
